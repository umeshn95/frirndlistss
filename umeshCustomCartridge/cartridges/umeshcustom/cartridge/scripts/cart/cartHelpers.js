...
function addLineItem(
   ...
   // productLineItem.custom.seteEngravingMessage("eeeeeeeeeeeeeeeeeeeeeeeee")  // #sachinRemoveThis
    try {
        productLineItem.custom.engravingMessage ="Happy Birthday BHAI :)";
    } catch (error) {
...
function getQtyAlreadyInCart(productId, lineItems, uuid) {
    ...
    collections.forEach(lineItems, function (item) {
        ...
        } else if (item.productID === productId && excludeUuid(uuid, item.UUID) && false) {
...
function addProductToCart(currentBasket, productId, quantity, childProducts, options) {
    ...
    if (productInCart && productInCart.custom.engravingMessage === options[0].engravingMessage) {
